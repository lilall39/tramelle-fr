...
export function PublierForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (category === "article") {
        if (coverFile) {
          const url = await uploadSubmissionImage(uid, coverFile, "submissions");
          coverImage = url;
          imageUrl = url;
        }
      } else if (mainFile) {
        imageUrl = await uploadSubmissionImage(uid, mainFile, "submissions");
      }
      ...
      setError("Envoi impossible. Vérifiez les champs et votre connexion.");
    } finally {
      setPending(false);
    }
  }
      ...
      <form onSubmit={onSubmit} className="mt-10 space-y-8">
        ...
        {category === "vente" ? (
          <fieldset className="space-y-3">
            ...
            <label className="block text-sm font-bold text-ink">
              Photo
              <input type="file" accept="image/*" onChange={(e) => setMainFile(e.target.files?.[0] ?? null)} className="mt-1 text-sm" />
            </label>
          </fieldset>
        ) : null}
        ...
        {category === "article" ? (
          <fieldset className="space-y-3">
            ...
            <label className="block text-sm font-bold text-ink">
              Image de couverture
              <input type="file" accept="image/*" onChange={(e) => setCoverFile(e.target.files?.[0] ?? null)} className="mt-1 text-sm" />
            </label>
          </fieldset>
        ) : null}
